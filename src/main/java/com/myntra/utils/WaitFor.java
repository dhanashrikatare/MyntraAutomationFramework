...
/** * WaitFor
 * ────────
 * All waiting methods are kept here in one place.
 *
 * WHY we need waits:
 *   Websites take time to load. If we try to click a button
 *   before it appears on screen, the test will fail.
 *   These methods WAIT until the element is ready, then return it.
 *
 * Usage (called from KeyWord.java):
 *     WaitFor.visibilityOfelement(driver, element);
 *     WaitFor.elementToBeClickaBle(driver, element);
 */
